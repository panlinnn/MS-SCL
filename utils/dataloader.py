...
s_road_path = 'road_map/xian1.png'
t_road_path = 'road_map/cdu1.png'
...
class MapData(data.Dataset):
    def __init__(self, datapath, mode='train', road_mode='no', channel=2):
        ...
        if self.road_mode == 'xian':
            ...
            self.road_map = np.expand_dims(
                cv2.resize(np.flip(np.array(PIL.ImageOps.invert(Image.open(s_road_path).convert('L'))), 0), (128, 128),
                           interpolation=cv2.INTER_LINEAR), 0)
        elif self.road_mode == 'cdu':
            print(f'current road map:{self.road_mode}')
            self.road_map = np.expand_dims(
                cv2.resize(np.flip(np.array(PIL.ImageOps.invert(Image.open(t_road_path).convert('L'))), 0), (128, 128),
                           interpolation=cv2.INTER_LINEAR), 0)
        elif self.road_mode == 'bj':
            print(f'current road map:{self.road_mode}')
            self.road_map = np.expand_dims(
                cv2.resize(np.array(PIL.ImageOps.invert(Image.open(bei_road_path).convert('L'))), (128, 128),
                           interpolation=cv2.INTER_LINEAR), 0)
        else:
